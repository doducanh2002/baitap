...
public class UserRestControllerImpl implements UserRestController {
    ...
    public SystemResponse<UserDTO> getUserByUsername(@RequestParam("username") String username) {
        User user;
        try {
            user = getUserByUsernameUseCase.execute(username);
    ...
    public SystemResponse<UserDTO> getUserByEmail(@RequestParam("email") String email) {
        User user;
        try {
            user = getUserByEmailUseCase.execute(email);
        } catch (UserNotFoundException ex){
            throw new EntityNotFoundException(ex.getMessage());
        }
        return SystemResponse.<UserDTO>builder()
